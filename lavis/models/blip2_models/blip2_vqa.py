...
@registry.register_model("blip2_vqa")
class Blip2VQA(Blip2Base):
    """
    BLIP2 Vicuna model.
    ...
    def __init__(
        ...
        # print(opt_model)
        self.opt_tokenizer = AutoTokenizer.from_pretrained(opt_model, use_fast=False, truncation_side="left")
        # self.llm_tokenizer = LlamaTokenizer.from_pretrained(llm_model, use_fast=False, truncation_side="left")
        self.opt_model = OPTForCausalLM.from_pretrained(
            opt_model, load_in_4bit=True
        )
        self.opt_tokenizer.add_special_tokens({'pad_token': '[PAD]'})
        self.opt_tokenizer.add_special_tokens({'bos_token': '</s>'})
        self.opt_tokenizer.add_special_tokens({'eos_token': '</s>'})
        self.opt_tokenizer.add_special_tokens({'unk_token': '</s>'})
    ...
    def forward(self, samples):
        ...
        inputs_llm = self.opt_proj(query_output.last_hidden_state[:,:query_tokens.size(1),:])
        ...
        text_output_tokens = self.opt_tokenizer(
            [t + self.opt_tokenizer.eos_token for t in samples['text_output']],
            return_tensors="pt",
            padding="longest",
            truncation=True,
    ...
    def generate(
        ...
        if image.dim() == 5:
            ...
            for j in range(image.size(2)):
                ...
                frame_inputs_llm = self.opt_proj(frame_query_output.last_hidden_state[:,:query_tokens.size(1),:])
    ...
    def _predict_class(
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            for n in range(n_segments):
                ...
                this_output_tokens = self.opt_tokenizer(
                ...
                inputs_embeds = self.opt_model.get_input_embeddings()(this_llm_input_ids)
                ...
                this_targets = this_llm_input_ids.masked_fill(this_llm_input_ids == self.opt_tokenizer.pad_token_id, -100)
